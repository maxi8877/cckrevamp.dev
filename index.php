<?php include( 'header.php'); ?>
  <section id="hero">
    <div class="swiper-container">
      <div class="swiper-wrapper">
        <!-- <div class="swiper-slide">
        <img src="https://placeholdit.imgix.net/~text?txtsize=200&txt=16:9&w=1600&h=900" />
        <div class="caption">
          <div class="in-frame">
            <h1>Unipersonales de Cristina Banegas: </h1>
            <p>Molly Bloom de James Joyce </p>
            <div class="tag-infancia">Infancia</div>
            <span class="caption-bg"></span>
            <a href="#"></a>
          </div>
        </div>
      </div>
      <div class="swiper-slide">
        <img src="https://placeholdit.imgix.net/~text?txtsize=200&txt=16:9&w=1600&h=900" />
        <div class="caption">
          <div class="in-frame">
            <h1>Fernando Rubio Fernando Rubio Fernando Rubio</h1>
            <p>Todo lo que está a mi lado</p>
            <div class="tag-informacion-general">Informacion General </div>
            <span class="caption-bg"></span>
            <a href="#"></a>
          </div>
        </div>
      </div> -->
        <div class="swiper-slide">
          <video  id="slider-video"  preload="auto" width="100%" autoplay muted="true" muted loop>
            <source src="//betterhidemyip.com//videos/video-drone-mute-720.mp4" type="video/mp4">
            <source src="//betterhidemyip.com//videos/video-drone-mute-720.ogv" type="video/ogg">
            <source src="//betterhidemyip.com//videos/video-drone-mute-720.webm" type="video/webm">
          </video>
          <div class="caption">
            <div class="in-frame">
              <h1>Centro Cultural kirchner</h1>
              <p>El Corazón de la Cultura </p>
              <a href="#"></a>
            </div>
          </div>
        </div>
        <div class="swiper-slide">
          <img src="https://placeholdit.imgix.net/~text?txtsize=200&txt=16:9&w=1600&h=900" />
          <div class="caption">
            <div class="in-frame">
              <h1>Fernando Rubio</h1>
              <p>Todo lo que está a mi lado</p>
              <div class="tag-muestraspermanentes">muestras permanentes </div>
              <span class="caption-bg"></span>
              <a href="#"></a>
            </div>
          </div>
        </div>

      </div>
      <div class="swiper-pagination"></div>
      <div class="downarrow"><i class="fa fa-arrow-circle-o-down"></i></div>
    </div>
    <section id="content" class="home">
      <div class="in-frame">
        <ul id="masonry-container">
          <li class="item article-item masonry-quarter">
            <div class="separator"></div>
            <article id="" class="article-evento cat-infancia">
              <header>
                <h4>
                  <strong>iinfancia</strong>
                  <small>el 31 de Agosto</small>
                </h4>
                <h1>
                  <a href="#">
                    <strong>Fernando Rubio:</strong>Todo lo que esta a mi lado</a>
                </h1>
                <p>
                  Obra móvil alrededor del mundo: Interviniendo diversos espacios de la ciudades con una serie de camas. En cada una de ellas se encuentra una actriz y un espectador.</p>
              </header>
              <figure>
                <img src="https://placeholdit.imgix.net/~text?txtsize=100&txt=4:3&w=800&h=600" />
                <figcaption></figcaption>
              </figure>
              <footer>
                <div class="compartir">
                  <a href="#"><i class="fa fa-twitter"></i></a>
                  <a href="#"><i class="fa fa-facebook"></i></a>
                </div>
              </footer>
            </article>
          </li>
          <li class="item article-item masonry-quarter">
            <div class="separator"></div>
            <article class="article-evento cat-informacion-general">
              <header>
                <h4>
                  <strong>Información General</strong>
                </h4>
              </header>
              <div class="flex-video">
                <iframe width="100%" height="315" src="https://www.youtube.com/embed/PtBq5nsD-8E?rel=0" frameborder="0" allowfullscreen></iframe>
              </div>
              <h3>
                <strong>Sophie Calle</strong>, Cuídese mucho
              </h3>
              <footer>
                <div class="compartir">
                  <a href="#"><i class="fa fa-twitter"></i></a>
                  <a href="#"><i class="fa fa-facebook"></i></a>
                </div>
              </footer>
            </article>
          </li>
          <li class="item article-item masonry-calendar">
            <div class="separator"></div>
            <article class="calendar">
              <header>
                <h4>
                  <strong>AGENDA</strong>
                </h4>
                <h5>Calendario</h5>
                <div id="calendar">
                </div>
                <input type="text" placeholder="email">
                <button>SUSCRIBIRSE</button>
                <footer>
                  <small>
                    Lorem Ipsum es simplemente el texto de relleno de las imprentas y archivos de texto. Lorem Ipsum ha sido el texto de relleno.
                  </small>
                </footer>
            </article>
          </li>
          <li class="item article-item masonry-quarter">
            <div class="separator"></div>
            <article class="article-evento ">
              <header>
                <h4>
                  <strong>LO QUE VIENE</strong>
                </h4>
                <img src="https://placeholdit.imgix.net/~text?txtsize=100&txt=4:3&w=800&h=600" />
                <h3>
                  <strong>Unipersonales de Cristina Banegas:</strong> Molly Bloom de James Joyce</h3>
                <small>
                  Lorem Ipsum es simplemente el texto de relleno de las imprentas y archivos de texto. Lorem Ipsum ha sido el texto de relleno.
                </small>
              </header>

              <footer>
                <div class="compartir">
                  <a href="#"><i class="fa fa-twitter"></i></a>
                  <a href="#"><i class="fa fa-facebook"></i></a>
                </div>
              </footer>
            </article>
          </li>
          <li class="item article-item masonry-quarter">
            <div class="separator"></div>
            <article class="article-twitter">
              <header>
                <h3><i class="fa fa-twitter"></i> @cckirchner</h3>
                <h4>

                </h4>
              </header>
              <p>Ya abrimos nuestras puertas para que vengan a disfrutar, conocer y aprender sobre nuestra cultura. #gratis</p>
              <img src="https://placeholdit.imgix.net/~text?txtsize=100&txt=4:3&w=800&h=600" />
              <hr>
              <h4>
                <small>27 Jun</small>
              </h4>
              <p>¿Sos docente o investigador? Acordate que ya podés anotar tu actividad y que sea parte de la #SemanaDeLaCiencia</p>
              <p class="text-center">
                <button><i class="fa fa-twitter"></i> follow @cckirchner</button>
              </p>
              <footer>
              </footer>
            </article>
          </li>
          <li class="item article-item masonry-quarter">
            <div class="separator"></div>
            <article class="article-gallery">
              <header>
                <h4>
                  <strong>GALERIA</strong>
                </h4>
              </header>
              <img src="https://placeholdit.imgix.net/~text?txtsize=100&txt=4:3&w=800&h=600" />
              <h3>La ballena azul</h3>
              <button>VER MÁS</button>
              <footer>
              </footer>
            </article>
          </li>
          <li class="item article-item masonry-quarter">
            <div class="separator"></div>
            <article class="article-evento-type-2 cat-escenicas">
              <header>
                <h4>
                  <strong>artes escénicas</strong>
                </h4>
                <h1>Así se baila el tango</h1>
              </header>
              <img src="https://placeholdit.imgix.net/~text?txtsize=100&txt=4:3&w=800&h=600" />
              <footer>
                <div class="compartir">
                  <a href="#"><i class="fa fa-twitter"></i></a>
                  <a href="#"><i class="fa fa-facebook"></i></a>
                </div>
              </footer>
            </article>
          </li>
          <li class="item article-item masonry-quarter">
            <div class="separator"></div>
            <article class="article-gallery-2">
              <header>
                <h4>
                  <strong>GALERIA</strong>
                </h4>
              </header>
              <img src="https://placeholdit.imgix.net/~text?txtsize=100&txt=4:3&w=800&h=600" />
              <div class="over-image">
                <h3>Como se construyó el CCK</h3>
                <button>VER MÁS</button>
              </div>
              <footer>
              </footer>
            </article>
          </li>
          <li class="item article-item masonry-quarter">
            <div class="separator"></div>
            <article class="article-evento-type-2 cat-escenicas">
              <header>
                <h4>
                  <strong>artes escénicas</strong>
                </h4>
                <h1>Sueña que duerme en el fondo del mar</h1>
              </header>
              <img src="https://placeholdit.imgix.net/~text?txtsize=100&txt=4:3&w=800&h=600" />

              <footer>
                <div class="compartir">
                  <a href="#"><i class="fa fa-twitter"></i></a>
                  <a href="#"><i class="fa fa-facebook"></i></a>
                </div>
              </footer>
            </article>
          </li>
        </ul>
      </div>
    </section>
    </div>
    <?php include( 'footer.php'); ?>




      <!--
     <li>
        <a class="icon-social soc-instagram" href="#"></a>
        <a href="#">instagram.com/cckirchner</a>
      </li>

    -->
